    ...
    while (true) {
        ...
        if (wifiConnected && mqqtConnected) {
            ...
            if (!primeiraPublicacaoFeita) {
                if (xQueueReceive(tempQueue, &temperature, 0)) {
                    snprintf(mensagem, sizeof(mensagem), "%d", temperature);
                    printf("[MQTT Task] Publicando temperatura inicial: %s\n", mensagem);
                    mqtt_comm_publish(CANAL_DO_BROKER_TEMP, (uint8_t*)mensagem, strlen(mensagem));
            ...
            if (xQueueReceive(joystickQueue, &joystickDirection, 0)) {
                ...
                mqtt_comm_publish(CANAL_DO_BROKER_JOYSTICK, (uint8_t*)mensagem, strlen(mensagem));
